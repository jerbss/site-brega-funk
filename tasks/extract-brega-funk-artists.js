...
// Chaves da API
const CLIENT_ID = process.env.SPOTIFY_CLIENT_ID || '7063fd2601e5422d8b55d9b0534af358';
const CLIENT_SECRET = process.env.SPOTIFY_CLIENT_SECRET || 'd548916124734c979b3c3443e30d12ec';
...
async function main() {
  ...
  const files = fs.readdirSync(CHARTS_DIR).filter(f => f.endsWith('.csv')).sort();
  ...
  console.log("3. Buscando dados das faixas (lotes de 50) para coletar IDs dos artistas...");
  for (let i = 0; i < trackIdsArr.length; i += 50) {
    const chunk = trackIdsArr.slice(i, i + 50);
    const data = await fetchWithRetry(`https://api.spotify.com/v1/tracks?ids=${chunk.join(',')}`, token);
    if (data && data.tracks) {
  ...
  const bregaKeywords = ['brega funk', 'funk pernambucano', 'batidao romantico'];

  for (let i = 0; i < artistIdsArr.length; i += 50) {
    const chunk = artistIdsArr.slice(i, i + 50);
    const data = await fetchWithRetry(`https://api.spotify.com/v1/artists?ids=${chunk.join(',')}`, token);
    
    if (data && data.artists) {
      data.artists.forEach(artist => {
        if (!artist) return;
        const genres = artist.genres || [];
        const isBregaFunk = genres.some(g => bregaKeywords.some(bk => g.toLowerCase().includes(bk)));
        if (isBregaFunk) {
  ...
  for (let year = 2018; year <= 2025; year++) {
    statsByYear[year] = { uniqueTracks: new Set(), appearances: 0, artistsFreq: {} };
  }
