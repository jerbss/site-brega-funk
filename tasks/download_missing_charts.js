...
(async () => {
	...
	console.log(
		`📅 Total de semanas ausentes a tentar obter: ${missingDates.length}`
	);
	...
	async function processDate(date) {
		return new Promise((resolve) => {
			...
			const cleanupAndResolve = () => {
				...
				completedCount++;
				console.log(
					`⏳ Progresso Faltantes: ${completedCount}/${missingDates.length} (${((completedCount / missingDates.length) * 100).toFixed(1)}%)`
				);
	...
	console.log(
		"🎉 Concluído! Descompacte o conteúdo deste ZIP de faltantes dentro da pasta 'C:\\site-brega-funk\\data-raw\\charts'."
	);
})();
