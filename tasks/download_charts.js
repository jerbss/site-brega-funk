...
(async () => {
	...
	async function processDate(date) {
		return new Promise((resolve) => {
			...
			const cleanupAndResolve = () => {
				...
				completedCount++;
				console.log(
					`⏳ Progresso: ${completedCount}/${dates.length} (${((completedCount / dates.length) * 100).toFixed(1)}%)`
				);
				resolve();
			};
			...
			iframe.onload = () => {
				...
				const interval = setInterval(() => {
					...
					const btn =
						iframeDoc.querySelector('button[aria-labelledby="csv_download"]') ||
					...
					if (btn) {
						...
						setTimeout(() => {
							if (!isResolved) {
								console.log(
									`⚠️ Blob não capturado para ${date}, tentando ler o DOM da tabela...`
								);
								try {
									const rows = iframeDoc.querySelectorAll('tr, [role="row"]');
									if (rows.length > 5) {
										let tableCsv =
											"position,track_name,artist_name,streams,id\n";
										rows.forEach((row, index) => {
											...
											if (cells.length >= 3) {
												...
												const artistName =
													...
													"";
												const streams = cells[2].textContent
													.replace(/[^0-9]/g, "")
													.trim();
												const trackLink =
													cells[1].querySelector("a")?.href || "";
												const trackId =
													trackLink.split("/track/")[1]?.split("?")[0] || "";
												if (rank && trackName) {
					...
					if (attempts > 30) {
						// 15 segundos esperando o botão
						clearInterval(interval);
						console.log(
							`❌ Timeout esperando botão de download na data: ${date}`
						);
						cleanupAndResolve();
					}
				}, 500);
	...
	console.log(
		"🎉 Download do ZIP concluído! Coloque este arquivo na pasta 'data-raw/charts/' e descompacte os arquivos lá dentro."
	);
})();
